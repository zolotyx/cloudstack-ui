...
export class AffinityGroupService extends BaseBackendCachedService<AffinityGroup> {
  ...
  public getTypes(params?: {}): Observable<Array<AffinityGroupType>> {
    return this.sendCommand('list;Types', params).map(
      result => result[`affinityGroupType`] || []
    );
  }
  ...
  public create(params: AffinityGroupCreationData): Observable<AffinityGroup> {
    return super
      .create(params)
      .switchMap(job =>
        this.asyncJob.queryJob(job.jobid, this.entity, this.entityModel)
      );
  }
  ...
  public updateForVm(
    ...
    return this.sendCommand('updateVM', {
      ...
      this.asyncJob.queryJob(job.jobid, 'virtualmachine', VirtualMachine)
    );
  }
