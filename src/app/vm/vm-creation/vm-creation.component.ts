...
export class VmCreationComponent implements OnInit {
  ...
  public get showResizeSlider(): boolean {
    return !!this.vmCreationState.state.template
      && (!this.vmCreationState.state.template.isTemplate || this.vmCreationState.state.showRootDiskResize);
  }
  ...
  public changeDiskOffering(diskOffering: DiskOffering) {
    this.diskOfferingChange.emit(diskOffering);
  }
